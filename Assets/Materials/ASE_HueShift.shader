...
Shader "ASE_Wall"
{
	Properties
	{
		...
		[ASEEnd]_TilingY("TilingY", Float) = 0
	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float2 texCoord62 = packedInput.ase_texcoord5.xy * _Vector3 + float2( 0,0 );
				float4 appendResult75 = (float4(_TilingX , _TilingY , 0.0 , 0.0));
				float2 texCoord60 = packedInput.ase_texcoord5.xy * appendResult75.xy + float2( 0,0 );
				float4 lerpResult66 = lerp( tex2D( _TextureSample1, texCoord62 ) , tex2D( _TextureSample0, texCoord60 ) , _Lerp);
				float3 hsvTorgb33 = RGBToHSV( lerpResult66.rgb );
				float3 hsvTorgb36 = HSVToRGB( float3(( hsvTorgb33.x + ( _TimeParameters.x * _Float1 ) ),hsvTorgb33.y,hsvTorgb33.z) );
				
				surfaceDescription.Albedo = hsvTorgb36;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				BuiltinData builtinData;
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 texCoord62 = packedInput.ase_texcoord.xy * _Vector3 + float2( 0,0 );
				float4 appendResult75 = (float4(_TilingX , _TilingY , 0.0 , 0.0));
				float2 texCoord60 = packedInput.ase_texcoord.xy * appendResult75.xy + float2( 0,0 );
				float4 lerpResult66 = lerp( tex2D( _TextureSample1, texCoord62 ) , tex2D( _TextureSample0, texCoord60 ) , _Lerp);
				float3 hsvTorgb33 = RGBToHSV( lerpResult66.rgb );
				float3 hsvTorgb36 = HSVToRGB( float3(( hsvTorgb33.x + ( _TimeParameters.x * _Float1 ) ),hsvTorgb33.y,hsvTorgb33.z) );
		...
		Pass
		{
						...
						)
			{
				...
				GlobalSurfaceDescription surfaceDescription = (GlobalSurfaceDescription)0;
				float2 texCoord62 = packedInput.ase_texcoord7.xy * _Vector3 + float2( 0,0 );
				float4 appendResult75 = (float4(_TilingX , _TilingY , 0.0 , 0.0));
				float2 texCoord60 = packedInput.ase_texcoord7.xy * appendResult75.xy + float2( 0,0 );
				float4 lerpResult66 = lerp( tex2D( _TextureSample1, texCoord62 ) , tex2D( _TextureSample0, texCoord60 ) , _Lerp);
				float3 hsvTorgb33 = RGBToHSV( lerpResult66.rgb );
				float3 hsvTorgb36 = HSVToRGB( float3(( hsvTorgb33.x + ( _TimeParameters.x * _Float1 ) ),hsvTorgb33.y,hsvTorgb33.z) );
...
WireConnection;36;2;33;3
WireConnection;14;0;36;0
ASEEND*/
//CHKSM=FCB3A1C00035E64F7C06A2A13DD9CD2C7FCC017F